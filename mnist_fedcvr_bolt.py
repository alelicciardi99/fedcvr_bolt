...
#alpha = torch.ones(K, device=device) / K  # Uniform distribution over clients
...
#D = sum(p.numel() for p in global_model.parameters())  # Dimensionality of model vector
D = 300
print(D)
num_pars = sum(p.numel() for p in global_model.parameters())
sampled_weights = random.sample(range(num_pars - 2000, num_pars), D)
